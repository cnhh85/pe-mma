...
import ActractiveDetail from "./ActractiveDetailScreen";
const Stack = createNativeStackNavigator();

export default function ActractiveScreen() {
  ...
  const Feed = ({ navigation }) => {
    const goDetail = (item) => {
      navigation.navigate("ActractiveDetail", { id: item.id });
    };
    const addFav = (item) => {
      if (item.isFavorite) {
    ...
    return (
      <>
        <SafeAreaView style={styles.container}>
          <ScrollView >
            {newState
              .filter((item) => item.status && item.actractive)
              .map((item) => {
  ...
  return (
    <>
      <Stack.Navigator header={header}>
        ...
        />
        <Stack.Screen
          name="ActractiveDetail"
          component={ActractiveDetail}
          options={{ headerShown: true }}
        />
      </Stack.Navigator>
    </>
